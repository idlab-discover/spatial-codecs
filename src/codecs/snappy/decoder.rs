//! Snappy wrapper decoder utilities.
...
fn payload(data: &[u8]) -> Result<&[u8], Box<dyn std::error::Error>> {
    if data.len() < 3 || &data[0..3] != b"SNP" {
        return Err("Invalid SNP header".into());
    }
    Ok(&data[3..])
}

/// Decompress into `Vec<P>` using the inner codec.
pub fn decode_into<P>(data: &[u8], out: &mut Vec<P>) -> BasicResult
where
    P: SpatialSink + 'static,
{
    let mut dec = snap::read::FrameDecoder::new(payload(data)?);
